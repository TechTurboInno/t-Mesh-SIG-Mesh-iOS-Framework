...
class MeshProvisioningDataTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    private func updateProvisioningDataUI() {
        ...
        let label1 = UILabel(frame:CGRect(x:20,y:13,width:200,height:20));
        label1.textAlignment = NSTextAlignment.right
        label1.text = "";
        label1.font = UIFont.systemFont(ofSize: 13);
        nodeNameCell.accessoryView = label1
...
//        }
        ...
        let label : UILabel = nodeNameCell?.accessoryView as! UILabel
        let info = targetNode.getDeviceInfomation()
        var data : Data = Data()
        data.append(contentsOf: info[3..<5])
        label.text = String(format: "Device Infomation:%@", data.hexString())
    }
    ...
    func nodeProvisioningCompleted(_ aNode: UnprovisionedMeshNode) {
        ...
        targetNode.shouldDisconnect()
        stepCompleted(withIndicatorState: true)
        meshManager.setCurrentNodeUnicast(targetNodeUnicast!)
...
extension MeshProvisioningDataTableViewController: ProvisionedMeshNodeDelegate {
    ...
    func bindModelKeys(_ type: Int) {
        var aModel : Data
        aModel = Data()
        if type == 1 {
            aModel = Data([0x10, 0x00])
        } else if type == 2 {
            aModel = Data([0x13, 0x00])
        } else if type == 3 {
            aModel = Data([0x13, 0x07])
        }
        
        let appKeyIndexData = Data([0x00, 0x00])
        
        let unicast = meshManager.currentUnicast()
        
        self.targetProvisionedNode.bindAppKey(withIndex: appKeyIndexData,
                                              modelId: aModel,
                                              elementAddress: unicast!,
                                              onDestinationAddress: unicast!)
        
        print("Binding appkey \(appKeyIndexData.hexString()) to Model \(aModel.hexString())")
        
        let aNodeEntry =  stateManager.state().provisionedNodes.last
        var anElement = aNodeEntry!.elements?[0]
        anElement?.setKeyBinding(appKeyIndexData, forModelId: aModel)
        
        aNodeEntry!.elements?.remove(at: 0)
        aNodeEntry!.elements?.insert(anElement!, at: 0)
