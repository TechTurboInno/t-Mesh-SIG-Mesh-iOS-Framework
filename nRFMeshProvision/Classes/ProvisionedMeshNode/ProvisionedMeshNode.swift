...
public class ProvisionedMeshNode: NSObject, ProvisionedMeshNodeProtocol {
    ...
    private var genericLightnessState: GenericModelControllerStateProtocol!
    private var genericHSLState: GenericModelControllerStateProtocol!
    private var genericCustomSetState: GenericModelControllerStateProtocol!
    private var genericCustomGetState: GenericModelControllerStateProtocol!
    private var genericGroupSetState: GenericModelControllerStateProtocol!
    private var genericGroupGetState: GenericModelControllerStateProtocol!
    ...
    public func nodeGenericSetLightness(_ anElementAddress: Data, onDestinationAddress anAddress: Data, withtargetState aState: Data) {
        let setState = GenericLightnessSetState(withTargetProxyNode: self,
                                                      destinationAddress: anAddress,
                                                      andStateManager: stateManager)
        setState.setTargetState(aTargetState: aState)
        genericLightnessState = setState
        genericLightnessState.execute()
    }
    
    public func nodeGenericSetHSL(_ anElementAddress: Data, onDestinationAddress anAddress: Data, withtargetState aState: Data) {
        let setState = GenericHSLSetState(withTargetProxyNode: self,
                                                destinationAddress: anAddress,
                                                andStateManager: stateManager)
        setState.setTargetState(aTargetState: aState)
        genericHSLState = setState
        genericHSLState.execute()
    }
    
    public func nodeGenericSetCustom(_ anElementAddress: Data, onDestinationAddress anAddress: Data, withtargetState aState: Data) {
        let setState = GenericCustomDataSetState(withTargetProxyNode: self,
                                          destinationAddress: anAddress,
                                          andStateManager: stateManager)
        setState.setTargetState(aTargetState: aState)
        genericCustomSetState = setState
        genericCustomSetState.execute()
    }
    
    public func nodeGenericGetCustom(_ anElementAddress: Data, onDestinationAddress anAddress: Data) {
        let getState = GenericCustomDataGetState(withTargetProxyNode: self,
                                                 destinationAddress: anAddress,
                                                 andStateManager: stateManager)
        genericCustomGetState = getState
        genericCustomGetState.execute()
    }
    
    public func nodeGenericSetGroup(_ anElementAddress: Data, onDestinationAddress anAddress: Data, withtargetState aState: Data) {
        let setState = GenericGroupSetState(withTargetProxyNode: self,
                                                 destinationAddress: anAddress,
                                                 andStateManager: stateManager)
        setState.setTargetState(aTargetState: aState)
        genericGroupSetState = setState
        genericGroupSetState.execute()
    }
    
    public func nodeGenericGetGroup(_ anElementAddress: Data, onDestinationAddress anAddress: Data) {
        let setState = GenericGroupGetState(withTargetProxyNode: self,
                                            destinationAddress: anAddress,
                                            andStateManager: stateManager)
        genericGroupGetState = setState
        genericGroupGetState.execute()
    }
