...
class GenericCustomDataSetState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: GenericCustomDataSetMessage
        if let targetState = targetState {
            message = GenericCustomDataSetMessage(withTargetState: targetState)
        } else {
