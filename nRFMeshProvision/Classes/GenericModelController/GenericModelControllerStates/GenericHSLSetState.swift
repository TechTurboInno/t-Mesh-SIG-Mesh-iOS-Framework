...
class GenericHSLSetState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: GenericHSLSetMessage
        if let targetState = targetState {
            message = GenericHSLSetMessage(withTargetState: targetState)
        } else {
