...
class GenericLightnessSetState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: GenericLightnessSetMessage
        if let targetState = targetState {
            message = GenericLightnessSetMessage(withTargetState: targetState)
        } else {
