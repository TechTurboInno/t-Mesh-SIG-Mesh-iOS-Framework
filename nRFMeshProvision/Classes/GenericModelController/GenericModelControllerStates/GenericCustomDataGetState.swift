...
class GenericCustomDataGetState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        let message = GenericCustomDataGetMessage()
        //Send to destination
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is GenericCustomDataStatusMessage {
                    let status = result as! GenericCustomDataStatusMessage
                    target.delegate?.receivedGenericCustomDataStatusMessage(status)
                }
            } else {
